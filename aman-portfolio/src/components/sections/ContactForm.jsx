...
function ContactForm() {
  ...
  const handleSubmit = (event) => {
    ...
    const gmailWindow = window.open(`https://mail.google.com/mail/?${params.toString()}`, '_blank', 'noopener,noreferrer')
    if (!gmailWindow) window.location.href = `mailto:${recipient}?subject=${encodeURIComponent(trimmed.subject)}&body=${encodeURIComponent(body)}`
    setStatus('Gmail opened with your message prepared.')
  }
  return <form className="contact-form" noValidate onSubmit={handleSubmit}><div className="grid gap-5 sm:grid-cols-2"><Field fieldRef={fieldsRef} errors={errors} label="Name" name="name" onChange={update} value={values.name}/><Field fieldRef={fieldsRef} errors={errors} label="Email" name="email" type="email" onChange={update} value={values.email}/></div><Field fieldRef={fieldsRef} errors={errors} label="Subject" name="subject" onChange={update} value={values.subject}/><Field fieldRef={fieldsRef} errors={errors} label="Message" name="message" textarea onChange={update} value={values.message}/><button className="contact-submit" type="submit">Prepare Email <span aria-hidden="true">{'\u2709'}</span></button><p className="contact-form-helper">This will open Gmail with your message prepared. Review it and press Send.</p>{status && <p className="contact-form-status" aria-live="polite">{status}</p>}</form>
}
